...
function CreateLogTable(objArray, fields, theme, enableHeader) {
    ...
    for (var objid in array) {
        ...
        var id = object._id;
        var alt = i % 2 == 0 ? '' : 'alt';
        var dateStr = get_field_value(object, '@timestamp');
        var dateObj = new Date(Date.parse(dateStr) + tOffset);
        dateObj.setMilliseconds(parseFloat(dateStr.match(/\.(\d+)/)[1]) / 1000);
        var time = prettyDateString(dateObj.getTime());
...
function details_table(objid, theme) {
    ...
    for (index in obj_fields) {
        ...
        if(index == "@timestamp") {
            ...
            d.setMilliseconds(Math.floor(parseFloat(value.match(/\.(\d+)/)[1]) / 1000));
            var from = ISODateStringUTC(new Date(d.getTime() - 1000*60*60));
            var to = ISODateStringUTC(new Date(d.getTime()));
            var q = "@timestamp:[\""+from+"\" TO \""+to+"\"] AND " +
                "server:"+obj_fields["server"]+" AND " +
                "@source_host:"+obj_fields["@source_host"]+" AND " +
                "thread:"+obj_fields["thread"];
            buttons +=
                "<i class='jlink icon-large icon-arrow-down msearch' " +
                "data-action='!!' data-field='"+q+"'></i> ";

            var from = ISODateStringUTC(new Date(d.getTime()));
            var to = ISODateStringUTC(new Date(d.getTime() + 1000));
            var q = "@timestamp:[\""+from+"\" TO \""+to+"\"] AND " +
                "server:"+obj_fields["server"]+" AND " +
                "@source_host:"+obj_fields["@source_host"]+" AND " +
                "thread:"+obj_fields["thread"];
            buttons +=
                "<i class='jlink icon-large icon-arrow-up msearch' " +
                "data-action='!!' data-field='"+q+"'></i> ";
        } else if(index == "requestid") {
            buttons +=
                "<i class='jlink icon-large icon-signin msearch' " +
                "data-action='!' data-field='" + field + "'></i> ";
        }
...
function mSearch(field, value, mode) {
    ...
    if (!single && !ts && (pattern.test(queryinput) == true)) {
